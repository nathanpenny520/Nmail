...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│ [✉ 邮件] [✎ 回复张三…] [📊 每日摘要] [✦ AI 总管家] [⚙ 设置]      🔔 ✦ ▯ ⚙ ＋ │
│  └─ 唯一常驻基座    └─ 点击右侧小按钮才产生的页签（可关）        └─ 图标按钮区   │
├────────────┬─────────────────────┬───────────────────────────────────────────┤
│ 文件夹树    │  邮件列表            │  阅读区                                    │
...
- **无固定页签**（审核意见：固定页签体验不好）：唯一常驻基座是「邮件」页签（不可关闭，即 §4 资源管理器）。其余一切页面都**点击才产生标签页**，复用现设置的 PAGE_TABS 机制（打开过即留下、去重、可关闭、记忆在 localStorage）。
- **右侧图标按钮区**（常驻，紧凑小按钮）：
  - 🔔 通知铃（行为不变）
  - ✦ **AI 总管家**（§6 专门入口；有待审批动作/待审草稿时显示数字徽章——AI 停用时隐藏）
  - ▯ 每日摘要
  - ⚙ 设置
- **写信页签**：沿用现有 ComposeContext 多标签（含未保存圆点、关闭确认），由列表操作或标签条 `＋` 按钮产生；写信时主区覆盖为写信台（现有 keep-alive 机制不动）。
- 页签支持中键关闭、拖拽排序（增强）；页签条空间不足时横向滚动（现有行为）。
...
| 待审草稿 + 草稿箱 | **合并**为树「智能视图 ▸ 草稿」（§5.1），带计数徽章 |
...
| `/drafts`、`/mydrafts` | 301 式重定向到 `/?view=drafts`（旧两条路由并到同一合并视图） |
| `/archived` | 重定向到 `/`（已归档不再是全局视图；各账号 Archived 文件夹在树中直达） |
| `/digest`、`/assistant`、`/settings` | 不变（入口改为小按钮） |

涉及文件：`frontend/src/components/Layout.tsx`（重写：删侧栏，标签条改「基座 + 开启式页签 + 图标按钮区」）、`App.tsx`（路由表）、`pages/InboxPage.tsx`/`ArchivedPage.tsx`（合并进新 `pages/MailPage.tsx`）。
...
- **合并视图**：树「智能视图 ▸ 草稿」（徽章=待审+编辑中数）→ 统一列表，分段筛选：**待审（AI）｜编辑中｜定时中｜已发送**；行操作按状态给：批准并发送 / 先编辑再发（打开写信页签载入该稿——AI 草稿从此可改后发）/ 改定时 / 丢弃 / 恢复。AI 待审稿与手写稿视觉以 `origin` 徽标区分（✦ AI / ✎ 手写）。
- **顺带收益**：定时发送对 AI 草稿同样可用；Agent 的 create_draft 工具与总管家里「仅存草稿」动作也落同一体系（§6.3/§6.5）。
- **兼容**：旧路由 `/drafts`、`/mydrafts` 重定向（§3.4）；前端 `DraftsPage`/`UserDraftsPage` 合并为 `DraftsHubPage`。
