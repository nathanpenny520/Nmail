...
v0.3.0 起内置公开桌面客户端凭证：**添加账号 → 输入邮箱地址 → 点「授权登录」→ 浏览器完成登录**，三步完成，无需去任何控制台注册应用。
...
- **Gmail（imap.gmail.com）直连被墙**：Nmail 自动跟随系统代理（和浏览器一致，无需设置）——代理工具开启「系统代理」即自动识别，设置-通用 状态行实时显示当前通道
- **代理工具必须处于运行状态并开启「系统代理」模式**——没开时 Gmail 授权/同步会失败（报错如实提示）
