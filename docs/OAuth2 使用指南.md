...
- **Gmail（imap.gmail.com）直连被墙**：
  1. 设置 → 通用 → **网络代理** 打开开关（地址默认自动检测系统代理，代理工具开了「系统代理」即自动识别；检测不到再手动填，如 `socks5://127.0.0.1:7890` 或 `http://127.0.0.1:7890`）
  2. **代理工具必须处于运行状态**——它没开时 Gmail 授权/同步会失败（报错如实提示）
- 开启后**所有账号**的收发与授权都走代理（和浏览器一致，无需按账号设置）；Outlook 及国内直连可达的邮箱走代理也能正常用
...
| `无法连接 XX 令牌服务 ... 10061` | 代理地址端口上没有服务在听 | 代理工具开起来，或修正地址；也可临时关掉代理开关（授权自动直连兜底） |
