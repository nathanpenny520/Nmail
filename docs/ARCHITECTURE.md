...
| `core/desktop.py` | 桌面图标一键安装（UPDATE_AND_DESKTOP.md §2）：Windows 桌面+开始菜单 .lnk（PowerShell COM）、macOS `~/Applications/Nmail.app`（Info.plist+icns+启动脚本）、Linux .desktop+hicolor；`nmail install-shortcut/uninstall-shortcut` 子命令同源 | 只写用户目录；启动目标优先同环境 console script（pip venv/uvx），源码直跑注入 backend 路径 `-c` 内联；状态记数据目录 `desktop-shortcut.json`，路径失效=不健康提示重装；图标资产随包分发（`backend/app/assets` + nmail.spec datas） |
