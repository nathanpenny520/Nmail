...
## nmail-cli/（独立包，REDESIGN_PLAN §19）

对外 API 的命令行客户端（独立 PyPI 包名 `nmail-cli`，`uvx nmail-cli@latest` 零安装；requests 为其运行依赖、测试走 ASGI 传输不依赖）：JSON envelope + exit code 契约、本机自动配对（经内部 /api/extkeys）、发送两阶段确认（exit 8 + summary）、`watch` NDJSON 新邮件流（`--timeout`/`--max-emails` 可自动退出，agent 调用防挂起）；配置 `~/.config/nmail-cli/config.json`（0600），环境变量 `NMAIL_BASE_URL/NMAIL_API_KEY` 可覆盖。面向 agent 的技能说明：仓库根 `skills/SKILL.md`。**版本线（2026-09-15 起）**：与 app 同一条版本线——`scripts/sync_version.py` 从根 pyproject 统一写入 nmail-cli/pyproject + `__init__.__version__` + SKILL.md（release.sh 自动调用，`test_version_sync.py` CI 兜底），CLI 版本协商 `_notice.update` 由此恢复正确语义。
...
| `api/user_drafts.py` | **草稿统一 API**（v0.4 P3）：CRUD + 附件上传/删除 + 定时/取消 + 发送 + AI 待审流转（丢弃/恢复/带指令重写/按邮件拟稿）+ 按状态批量清空（`DELETE /api/user-drafts?status=`，仅开放 sent/discarded 终态） | status 全集 editing/scheduled/pending_review/sent/discarded，origin 区分 ai/human；列表 LEFT JOIN in_reply_to 原邮件作上下文；发送走 `core/outbox.send_user_draft` 唯一通路（pending_review 同样适用）；`/regenerate-for-email` 供邮件视图一键拟稿；原 api/drafts.py 已退役删除（路由 `/api/drafts` 不复存在） |
...
| `api/ai.py` | 单邮件问答、总管家问答（会话持久化）、**Agent 对话**（§17 v2）、写作辅助、用量、AI 整理 | 问答/总管家均有 `/stream` SSE 版本；**Agent**：`POST /api/ai/agent/stream`（SSE 事件 run_started/text_delta/text/tool_call/tool_result/approval_required/paused/error/done，分段轨迹落会话 `segments_json`）、`/agent/resume`（续跑：审批决定/步数预算触顶/ask_user 澄清回答〔可选 answer〕后，决定结果回写前一条消息 waiting step）、`/agent/runs`（运行列表，跨刷新恢复「继续」入口——A6）+ `/agent/runs/{id}`（详情：状态/pending/ai_logs 步级 token 耗时——A8）、`/agent/action/{id}/decide`（批准/拒绝+权限复核+参数可改）、`/agent/action/{id}/undo`、`/agent/actions`（审计；DELETE 单条删 / `?scope=old|failed|all` 批量清——§18.3）、`/memory` GET+DELETE（跨会话用户偏好——§18.5）、`/proposals` GET（懒触发提议检查）+`/{id}/decide`（采纳入黑名单/忽略——§18.5）；`_build_segments` 事件→分段与前端同构；对话/写作接受 `profile_id`；AI 整理为异步 job |
...
| `api/ext.py` | **对外 API**（v0.4 P7，REDESIGN_PLAN §7）：`/api/ext/v1/*`——health（免认证，含 version 供 CLI 版本协商 _notice.update——B2）/accounts/emails（列表/搜索〔P1：sender/recipient/after/before/has_attachments 过滤〕/详情/附件/emails/recent 游标轮询〔P1 watch〕）/emails/actions（批量，慢动作返回 job_id 经 /jobs 轮询）/drafts（列表/单条〔P2，CLI 发送摘要用〕/创建〔P1 正文三选一 html/md/text〕/reply|forward 回复转发〔P1，对齐写信台语义〕/approve 发送/{id}/attachments 上传〔P1〕）/folders（+/sync 按需同步）/contacts/digest/agent（chat 非流式+SSE/decide 审批） | `require_key(scope)` 依赖：api_enabled 总开关（403）→ X-Api-Key sha256 查表（401）→ scope 校验（403）→ 60 次/分钟内存滑动窗 + 每 Key 每日上限（429）；端点全部薄壳转调内部实现（emails/user_drafts/folders/contacts/digest/agent 零新邮件操作）；agent 调用 origin=api 全量进 ai_actions；错误统一 envelope（main.py 异常处理器：/api/ext/* 失败体 {"ok":false,"error":{code,message}} + 429 Retry-After，内部 API 与 /api/extkeys 不受影响）；指南见 docs/对外API使用指南.md |
| `api/extkeys.py` | 对外 API 密钥管理（仅内部设置页，走常规来源校验） | 明文存 secrets.json（`ext_api_key:{id}`，所见即所存回显），表内只留 sha256 哈希；生成/改名/scope/每日上限（0=不限）/重置（旧串立即失效）/吊销（行保留供日志对账）+已吊销行彻底删除（DELETE 两段语义：活跃=吊销、已吊销=删行，日志回退「已删」，2026-09-15 用户反馈补）；`/enabled` 总开关+日志开关；`/calls` 调用日志（30 天保留） |
...
| `core/batch_ops.py` | 批量 IMAP 动作任务体（trash/move/archive/unarchive 异步化） | 按账号分组共用连接、进度按账号上报；R2 语义（拿不到新 UID 删行交增量重建）与「服务器成功才动本地」保持；archive 逐账号惰性建归档夹再移动；打标类在端点同步执行；成功归档/删除后回写 `core/rule_proposals.observe`（§18.5 规则提议观察，sender 移动前快照）。**v0.4.1**：移动类发生删行后就地复用同一 IMAP 连接增量同步目标文件夹（`sync.resync_folder_with_mb`，删行重建不再等下次轮询——动作后立即可读），并按 message_id 找回新 id 随 job 结果返回 `rebuilt: {旧id: 新id}`（QQ 等不回 COPYUID 的服务商上旧 id 会失效）；trash 不做重建（删除即本地消失是既有语义） |
| `core/rule_proposals.py` | 手动整理观察 → 黑名单规则提议（§18.5） | rule_observations 按 email_id 去重记录；同发件人 14 天 ≥3 次 → pending 提议（agent_proposals；已在名单/pending/rejected 不再提、pending 上限 5）；提议生成幂等（GET 列表懒触发兜底）；批准/拒绝在 api 层（批准转调 add_sender_list 既有管线） |
...
| `core/update_check.py` | 应用内更新检查：GitHub Releases 对比 + 通知中心提醒 | 仅匿名 GET api.github.com（UA=Nmail/版本），24h 缓存；开关 `update_check_enabled`；按 ref_id=版本去重，升级后自动清理旧提醒；`notify_ready()` 把就位版本的通知改写为「重启即更新」 |
| `core/channel.py` | 安装渠道识别：binary（Release 直装）/ brew / winget（冻结二进制按安装路径特征）/ pip / uvx（uv 缓存 archive-v0 特征） | 自更新与桌面集成的分流依据；`can_self_update()`（binary/pip）与 `upgrade_hint()`（不可自更新渠道的升级命令） |
| `core/desktop.py` | 桌面图标一键安装（UPDATE_AND_DESKTOP.md §2）：Windows 桌面+开始菜单 .lnk（PowerShell COM）、macOS `/Applications/Nmail.app`（无权限回退 ~/Applications；Info.plist+icns+编译型存根 nmail-stub 注册 NSApplication——纯脚本 bundle 不被 LaunchServices 注册即无 Dock 图标，2026-09-15 用户实测后修复；服务为存根子进程，Quit 信号链式停服）、Linux .desktop+hicolor；`nmail install-shortcut/uninstall-shortcut` 子命令同源 | 只写用户目录；启动目标优先同环境 console script（pip venv/uvx），源码直跑注入 backend 路径 `-c` 内联；状态记数据目录 `desktop-shortcut.json`，路径失效=不健康提示重装；图标资产随包分发（`backend/app/assets` + nmail.spec datas） |
| `core/update_apply.py` | 应用内自更新执行（UPDATE_AND_DESKTOP.md §3）：下载（进度节流落 KV）→ SHA256（Release 资产 digest）→ **换身**（运行中可执行文件可 rename 不可覆写：本体 rename *.old 后原路径原子落位，失败回滚还原）→ ready + 通知；`restart_app()` spawn `--wait-port` 接管同端口后本进程退出；`auto_update_tick()` 自动心跳（检查开启+自动安装开启+渠道可自更新+确有新版本才触发） | 换身仅冻结 binary 渠道；pip 渠道 `pip install --upgrade` 原地升级；cli 启动早段 `finish_pending_swap()` 收尾半程下载（全函数零抛错——首启 KV 表未建也不炸）；状态 KV `update_apply_state`，下载暂存 `<DATA_DIR>/update/nmail.new` |
| `ai/llm.py` | OpenAI 兼容客户端（流式 `iter_deltas` / 非流式）+ Agent 单步 `chat_step`/`iter_chat_step`（原生 function calling，tools+tool_calls 解析，流式分片按 index 聚合，§17.1） | connect 5s / 读写 15s 快速失败 |
...
| `ai/tools.py` | 总管家工具集（§17.3 对齐扩充） | 31 个工具（8 读 + 1 meta + 22 写），每个带 JSON Schema（原生协议）；ask_user（A5 澄清中断，kind=meta 不入审计、scheduler 白名单天然排除）与 read_skill（A7 内置工作流技能全文）；search_emails 支持分类/发件人/未读/日期过滤且默认覆盖全部文件夹（修 v1 锁死 INBOX），空结果带结构化 hint；新增 set_category/rename_folder/delete_folder/update_draft/schedule_draft/discard_draft/upsert_contact/delete_contact/add_sender_list/remove_sender_list/save_memory/list_memory/delete_memory（§18.5 跨会话记忆：evidence 必填=用户原话引用，同文去重，上限 100 条）；薄壳转调既有能力；**无任意 HTTP/文件系统/命令类工具，也无账号/凭据/授权位/密钥类工具（§17.3 豁免防注入自我扩权）** |
| `ai/agent.py` | 总管家 Agent 循环 v2（§17，可恢复） | **原生 function calling 优先**（探测降级 JSON 协议并按 base_url+model 缓存 KV；`_extract_json` 首个平衡对象止血）；**时间预算优先**（180s 超限 tool_choice=none 强制文本收尾，仍调工具则暂停）+ MAX_STEPS=25 兜底；**审批不断链**：写类出卡后 run 置 waiting_approval（agent_runs v22 持久化 messages/步数/预算），批准/拒绝经 decide 后由 `/agent/resume` 续跑（拒绝同样回灌让模型改道），步数/预算触顶前端「继续」=新的一段预算；工具结果分工具预算紧凑回灌（read_email 4000 其余 1200 字符）；**上下文管理 §17.8 五层**：L2 微压缩（步数/token 双门，早期结果→确定性摘要行，只缩不删保配对）+ L3 会话记忆（chat_sessions.memory_json 注入 system+动作台账增量回写，服务端自取历史）+ L5 AutoCompact（水位≥窗口 80% 调一次 LLM 压五段式摘要，原文归档 archived_json，KV agent_autocompact=0 可停）+ 溢出自愈（context overflow 报错紧急压缩重试一次）；每步流式 text_delta；`_feedback_text` 邮件列表转行格式；调度定时运行（§18.6 晨报）走 SCHEDULER_ALLOWED 工具白名单（schema 过滤+执行层双拦、allowed_json 随 run 持久化）；自动模式约束与 ai_actions 审计/undo 全保留（v1 见 §6）；**§20 扩展**：触顶先强制小结再 paused（A1）+ 最终回答完成断言校验（A2：声称已完成的写操作无本 run 执行记录→纠正一次/警示放行）+ 工具结果保头尾截断（A3）+ 同批全只读并行执行≤4 workers 原序回灌（A4）+ ask_user 澄清 waiting_input 暂停经 resume(answer) 续跑（A5）+ 系统提示词注入内置技能索引（A7，skills_builtin） |
| `db/database.py` | 连接（WAL）+ `MIGRATIONS` 版本化迁移（v25：rule_observations/agent_proposals/agent_runs.allowed_json；v24：agent_memory；v23：chat_sessions.memory_json、agent_runs.archived_json/summary_json）+ KV 设置 | 迁移只追加不改历史 |
| `scheduler.py` | 每 60s tick：到期账号增量同步 + 摘要到点生成 + 定时草稿派发 + AI 晨报分流（§18.6） | 「检查到期」而非每账号注册任务，改设置无需重建调度；定时草稿到期调 `core.outbox.send_user_draft`（不依赖 API 层），成功/失败写通知中心，失败退回 editing；轮询拉 INBOX + 归档文件夹（仅当该账号文件夹缓存里已有归档夹；v0.4.1——归档是真实服务器移动，归档夹不进轮询会让移动后删行重建的邮件迟迟不可见），其余文件夹按需同步（树选中触发）且不进 AI 管线（`sync.py` 仅 INBOX 新邮件进管线）；agent_brief_enabled 开启时 digest_time 到点改跑 AI 晨报（独立线程，origin=scheduler + SCHEDULER_ALLOWED 工具白名单，当天标记走 KV agent_brief_last_run）；晨报文本经 `digest.store_agent_brief` 写入当日摘要页（AI 综述=晨报正文，失败/无产出回退旧版 build_digest）；每小时 `update_tick` 兜底自动更新心跳（检查侧 24h 缓存节流，不可自更新/开关关闭时内部直返） |

## 前端（frontend/src/）

| 部分 | 内容 |
|------|------|
| `pages/` | MailPage（**邮件基座**：FolderTree + 视图组合——聚合收件箱/账号收件箱/服务器文件夹走 MailBrowser、**草稿=DraftsHubPage**（v0.4 P3：AI 待审+手写合并，分段筛选 待审/编辑中/定时中/已发送/已丢弃，origin 徽章，批准发送/编辑后发送/重写/丢弃/恢复/取消定时）；选中非 INBOX 文件夹触发按需同步；拖拽移动走 batchAction job；内置存量本地归档一次性迁移弹窗；视图初值取 URL，旧路由 /drafts /mydrafts → /?view=drafts、/archived → /）、DigestPage（ECharts 摘要）、ManagerPage（**AI 总管家 2.x §17**：对话 Agent——消息按 segments 渲染（流式 Markdown 文本 + 连续步骤合并的可折叠「执行过程」块：运行中自动展开/完成自动收起、行点开看参数明细、中文工具名映射 + 撤销钮；审批卡独立醒目带影响明细，批准/拒绝后自动续跑）、步数/预算触顶「继续」按钮、Stop 中断（AbortSignal）、错误内联红条；审批/自动模式开关、账号范围/档案选择不变）、SettingsPage（侧边栏分类：通用/邮箱账号（含 AI 权限面板=5 授权位+AI 专属邮箱）/通讯录（2026-09-12 改版：左树=智能视图+联系组右键管理、自动采集开关，右列表/详情双态，详情可写信（openNew 带初始收件人）/编辑/加入移出组）/AI 配置/AI 用量（含 AI 操作记录查看器）/API（v0.4 P7 对外 API：总开关+日志开关+密钥生成/重置/吊销+明文遮蔽回显/调用日志，ExtApiSection 组件）/关于） |
...
- 来源校验（main.py 中间件）：Host 必须为本机主机名（端口与实际监听一致才严格比对）；浏览器附带 Origin 时必须为本机源——挡恶意网页对 127.0.0.1 的 drive-by POST 与 DNS rebinding；携带 Cloudflare 边缘头（CF-*，只会经公网隧道注入——cloudflared 默认 Host 重写为本机地址 + 脚本客户端无 Origin，公网隧道下前两道双双失效）的请求一律 403（2026-09-15 审计 A1；SSH 无附加头、Tailscale serve 仅 X-Forwarded-* 均不受影响）。**例外**：`/api/ext/*`（对外 API，v0.4 P7）在来源守卫之前放行、改持 X-Api-Key 认证（外部脚本经自建隧道到达时 Host/Origin 本非本机；浏览器跨站带不上自定义头——预检不通，drive-by 由密钥兜住）；全部 ext 调用（含被拒的）落 api_calls（/health 除外）
...
| 发版脚本 | `scripts/release.sh` | 一条命令：`sync_version.py` 统一四处版本号（根 pyproject / nmail-cli / SKILL.md）→ 提交打 tag → 盯 CI → 自动提 winget 版本 PR；手册与踩坑见 `docs/RELEASE.md` |

已知分发注意点：Windows SmartScreen 对无签名 exe 会警告（缓解：onedir/误报申诉/买签名证书）；macOS 未公证二进制需右键打开或 `xattr -cr`（公证需 Apple Developer 账号）。**入口保护（2026-09-15 修复）**：cli.py 必须带 `if __name__ == "__main__": main()`——PyInstaller 以脚本原样执行入口，无保护则 exe 定义完函数即静默退出（v0.4.1 及之前的单文件二进制存在此隐患，873ce06 修复）；应用内自更新与桌面图标见 UPDATE_AND_DESKTOP.md（冻结 exe 换身 + `--wait-port` 端口接管，v0.4.2 起生效）。**Windows 无窗口化（UPDATE_AND_DESKTOP.md §6）**：nmail.spec `console=(sys.platform != "win32")`——Windows 双击不弹黑窗；配套 cli.py 日志落盘（`<DATA_DIR>/nmail.log` RotatingFileHandler，uvicorn 经 `log_config` 注入）+ 启动失败 traceback 落盘/Windows 原生 MessageBoxW 兜底；`/api/quit` + 设置页退出卡片为无窗口平台的显式停服入口。
