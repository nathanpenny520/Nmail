...
| `api/settings.py` | 通用设置 KV 读写 + AI 端点测试 | 含 `ui_font/body_font` 档位校验、`network_proxy` 代理地址即时校验 + `network_proxy_enabled` 总开关；GET 附带 `detected_proxy`（系统代理探测结果，供设置页展示）；AI 配置已移至 profiles；`/api/ai/test` 字段省略时回退激活档案 |
| `api/accounts.py` | 账号 CRUD/测试/探测/后台同步触发/服务商预设/文风提示词 | 授权码存 `secrets.json`（key=`account_pwd:{id}`）；`POST /accounts/probe` 未收录域名自动探测；代理为全局总开关（`core/netproxy`，无账号级字段）；OAuth 账号拒绝改密（400 指引重新授权）、删除账号时一并清 OAuth 令牌与 folders 缓存 ；列表/详情回传 `password` 明文（授权码所见即所存，同 AI key；仅本机 API——对外 ext 有独立窄 DTO 不含密码） |
