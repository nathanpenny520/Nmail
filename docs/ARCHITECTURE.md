...
| `api/accounts.py` | 账号 CRUD/测试/探测/后台同步触发/服务商预设/文风提示词/代理开关 | 授权码存 `secrets.json`（key=`account_pwd:{id}`）；`POST /accounts/probe` 未收录域名自动探测；`use_proxy` 账号级代理开关（经 `core/netproxy` 生效）；OAuth 账号拒绝改密（400 指引重新授权）、删除账号时一并清 OAuth 令牌与 folders 缓存 ；列表/详情回传 `password` 明文（授权码所见即所存，同 AI key；仅本机 API——对外 ext 有独立窄 DTO 不含密码） |
