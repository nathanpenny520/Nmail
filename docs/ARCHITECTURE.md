...
| `api/ai.py` | 单邮件问答、总管家问答（会话持久化）、**Agent 对话**（§17 v2）、写作辅助、用量、AI 整理 | 问答/总管家均有 `/stream` SSE 版本；**Agent**：`POST /api/ai/agent/stream`（SSE 事件 run_started/text_delta/text/tool_call/tool_result/approval_required/paused/error/done，分段轨迹落会话 `segments_json`）、`/agent/resume`（续跑：审批决定/步数预算触顶后，决定结果回写前一条消息 waiting step）、`/agent/action/{id}/decide`（批准/拒绝+权限复核+参数可改）、`/agent/action/{id}/undo`、`/agent/actions`（审计）；`_build_segments` 事件→分段与前端同构；对话/写作接受 `profile_id`；AI 整理为异步 job |
