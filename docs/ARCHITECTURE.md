...
| `api/accounts.py` | 账号 CRUD/测试/探测/后台同步触发/文件夹/服务商预设/文风提示词/代理开关 | 授权码存 `secrets.json`（key=`account_pwd:{id}`）；`POST /accounts/probe` 未收录域名自动探测；`use_proxy` 账号级代理开关（经 `core/netproxy` 生效）；OAuth 账号拒绝改密（400 指引重新授权）、删除账号时一并清 OAuth 令牌 |
