...
| agent_runs 自动清理 | ✅ 已落地（REDESIGN_PLAN §18.2/§18.3：终态 30 天随启动清理，waiting/paused 保留，running 僵尸对账） | 已完成 2026-09-15 |
| 超长任务 LLM 压缩 | ✅ 已由 §17.8 L5 AutoCompact 落地（token 水位触发，非步数条件） | 已完成 2026-09-14 |
| 步数/预算可配置 | 180s/25 步进设置页 | 用户反馈需要 |

### 11.2 能力增强（中期方向）

1. **跨会话记忆** ✅ 已落地（2026-09-15，REDESIGN_PLAN §18.5）：记住用户偏好与常用操作（"我的归档习惯""给谁发信要正式"），借鉴 inbox-zero 的
...
5. **自动化规则学习** ✅ 已落地（2026-09-15，REDESIGN_PLAN §18.5）：用户手动整理时 AI 观察并**提议**规则（"以后这类邮件自动归档？"），批准后由
   既有管线执行——自动规则与 agent 循环分离的架构已就位（借鉴 inbox-zero learned patterns，只借思想）。
6. **主动式助手** ✅ 已落地（2026-09-15，REDESIGN_PLAN §18.6）：定时指令（"每天 8 点总结未读并拟好需要回复的草稿"）——scheduler 触发 agent 运行，
   产出进通知中心；与每日摘要调度骨架结合（开关替代当日摘要）；无人值守场景以工具白名单硬边界替代逐项审批，草稿一律进待审列表。
