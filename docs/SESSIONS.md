...
- 续轮(同日): ①用户实测模板编辑见旧界面 → 根因 StaticFiles 无 Cache-Control 浏览器启发式缓存,8637844 中间件出口统一缓存头(index.html no-cache / assets immutable;`/` 的响应在 Mount 内部生成、路径形态判断三改仍漏,按响应类型判定无死角) ②审查卡条目跳变+像卡住 → 两阶段渐进(规则秒出/AI 追加只增不删)+单飞防重入,同提交。80a00dc 哈希回填
